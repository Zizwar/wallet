    ...
    return new Promise((resolve_, reject_) => {
        ...
        axios(params).then(function ({ data = [] }) {
            ...
            $.querySelectorAll('tr').forEach(function (el) {
                ...
                const quantity = el.querySelectorAll('td')[3] && el.querySelectorAll('td')[3].innerText.trim() || "not found";
                const price = el.querySelectorAll('td')[4] && el.querySelectorAll('td')[4].innerText.trim() || "not found";
                const priceBNB = price.substring(price.indexOf("(") + 1, price.lastIndexOf(")"))
                const symbole = el.querySelectorAll('td')[2] && el.querySelectorAll('td')[2].innerText.trim() || "not found";
                const profit = el.querySelectorAll('td')[7] && el.querySelectorAll('td')[7].innerText.trim() || "not found"
                const profitBNB = el.querySelectorAll('td')[6] && el.querySelectorAll('td')[6].innerText.trim() + " BNB" || "not found"
                
                tokens.push({ name, logo, token, priceBNB, price, symbole, quantity, profit, profitBNB })
